...
the canonical ensemble with inverse of temperature  β. With *f(S)* the logistic distribution with mean *c*. The function takes an array, promotes it to an
...
function contacthofield{T<:Real}(vec0::Array{T,1},c::T, beta::T)
  ...
  for k = 0:order-1
    ...
    #The order in the array D (and y) is D[1] = q, D[2] = p, D[3] = S. With an additional equation for the extended invariant
...
function contacthointegration!{T<:Real}(field::Function,nsampling::Int64, initcond::Array{T,1}, deltatsample::T, c::T, beta::T)
  ...
  invariant_initial = logrho(x, beta, c)
  ...
  invariant = Array(Float64, nsampling)

  #Saves the initial condition
  q[1] = x[1]
  p[1] = x[2]
  S[1] = x[3]
  invariant[1] = invariant_initial

  #Time array for sampling
  time = [deltatsample*(i-1) for i in 1:nsampling]
  temporarytime = 0.

  #Initial value of the extended variable that will be added to log(rho) along the flux
  V = 0
  push!(x, V)
  ...
  for i in 2:nsampling
    ...
    while j
      ...
      if time[i] < temporarytime
        ...
        p[i] = x[2]
        S[i] = x[3]
        invariant[i] = x[4] + logrho(x, beta, c)
        j = false
      end
