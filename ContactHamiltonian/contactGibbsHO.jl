...
function campoContacto{T<:Real}(x ::Array{T,1},c::Float64, beta::Float64)
  ...
  for k = 0:order-1
    ...
    #Campo de Gibbs de contacto con "c" libre: f(S) = exp(-c*S^4)

    D[1] = 1/4*pi*beta^(1/2)*2^(3/4)/gamma(3/4)^(1/2)*exp(1/2*c*y[3]^4+beta*y[1]^2+1/4*beta*y[2]^2)/c^(1/8)*y[2]
    D[2] =(c*y[2]*y[3]^3-beta*y[1])*pi*2^(3/4)/gamma(3/4)^(1/2)*exp(1/2*c*y[3]^4+beta*y[1]^2+1/4*beta*y[2]^2)/(c^(1/8)*beta^(1/2))
    D[3] = -1/4*pi*(beta*y[2]^2-2)*2^(3/4)/gamma(3/4)^(1/2)*exp(1/2*c*y[3]^4+beta*y[1]^2+1/4*beta*y[2]^2)/(c^(1/8)*beta^(1/2))
