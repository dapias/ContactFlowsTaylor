...
function campoContacto{T<:Real}(x ::Array{T,1}, c::Float64, beta::Float64)
  ...
  for k = 0:order-1
    ...
    #Campo de Tsallis de contacto con "c" libre: f(S) = exp(-c*S^4) y q (parámetro de extensividad) = 3/2.

    D[1] = 3/8*(4+(4*y[1]^2+y[2]^2)*beta)^(1/2)*exp(1/2*c*y[3]^4)*beta*y[2]
    D[2] =(1/4*beta*c*y[2]^3*y[3]^3+c*y[3]^3*(beta*y[1]^2+1)*y[2]-3/2*beta*y[1])*exp(1/2*c*y[3]^4)*(4+(4*y[1]^2+y[2]^2)*beta)^(1/2)
    D[3] = 1/4*(2*beta*y[1]^2-beta*y[2]^2+2)*(4+(4*y[1]^2+y[2]^2)*beta)^(1/2)*exp(1/2*c*y[3]^4)
